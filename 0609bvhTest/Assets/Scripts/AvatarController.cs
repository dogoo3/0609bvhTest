...
//     ]
    ...
    {
        // cutSamples = this.gameObject.GetComponent<SetRECDevice>().p_cutSamples;
...

    {
        ...
        Runtime.PythonDLL = @"C:\InstallProgram\Python\Python38\python38.dll";
        ...
        string PYTHON_HOME = @"C:\InstallProgram\Python\Python38";
            ...
            new string[] {
                  PythonEngine.PythonPath,
                     Path.Combine(PYTHON_HOME, @"Lib\site-packages"),
                      ...
                      @"D:\GithubProject\MDDFinal\MDD_Final\gesticulator",  // the root folder itself  under which demo package resides; demo package has demo.py module
                      @"D:\GithubProject\MDDFinal\MDD_Final\gesticulator\gesticulator",
                       @"D:\GithubProject\MDDFinal\MDD_Final\gesticulator\gesticulator\visualization"
        ...
        string audio_file = @"D:\GithubProject\gesticulator\demo\input\jeremy_howard.wav";

        // Start Coroutine: It basically execute a custom Update(), which has a yield statement somewhere
        
        StartCoroutine(GetSpeech_GenGesture_Display(audio_text, cutSamples, samplerate));

        cutSamples = recDevice.GetCutSamples();
        samplerate = recDevice.GetSampleRate();
    ...
    IEnumerator GetSpeech_GenGesture_Display(string audio_text, float[] audio_array, int samplerate)
    {
...
//
                ...
                dynamic motionPythonArray = democs.main2(audio_text: audio_text, audio_array: audio_array, sample_rate:samplerate);
               ...
               // This variable is set  to the list of children transforms of the bvh avatarRoot in Awake() method of the current component, AvatarController;
                ...
                yield return StartCoroutine(GetSpeech_GenGesture_Display(audio_text, audio_array, samplerate));
